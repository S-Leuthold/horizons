# tests/testthat/helper-select.R
# Fixture for select_training() and its internals: a reference pool with two
# spectral families and two responses (one sparse), plus a small batch of
# targets on a coarser grid, one of which is an exact twin of a pool row.


## ---------------------------------------------------------------------------
## gaussian_family() — one family of synthetic spectra
## ---------------------------------------------------------------------------

#' Build one family of synthetic absorbance spectra
#'
#' @description
#' Spectra are a sum of Gaussian peaks at family-specific centres, so rows of
#' one family sit closer to each other than to the other family in any
#' sensible similarity space.
#'
#' @param n [Integer.] Rows to build.
#' @param wn [Numeric.] Wavenumbers, decreasing.
#' @param centres [Numeric.] Peak centres (cm-1).
#' @param noise [Numeric.] SD of the additive noise. Default: `0.01`.
#'
#' @return [Matrix.] `n` by `length(wn)`.
#' @noRd
gaussian_family <- function(n, wn, centres, noise = 0.01) {

  ## Each peak gets its own per-sample height and width, so the family
  ## spans as many real components as it has peaks rather than one
  ## amplitude axis (SNV would collapse a single shared shape).

  m <- matrix(0, nrow = n, ncol = length(wn))

  for (cc in centres) {

    height <- stats::runif(n, 0.3, 1.2)
    width  <- stats::runif(n, 40, 90)
    shift  <- stats::rnorm(n, sd = 4)

    m <- m + height * exp(-((outer(rep(1, n), wn) - (cc + shift)) / width)^2)

  }

  m <- m + stats::runif(n, -0.05, 0.05)
  m + matrix(stats::rnorm(n * length(wn), sd = noise), nrow = n)

}


## ---------------------------------------------------------------------------
## make_select_fixture() — pool + targets
## ---------------------------------------------------------------------------

#' Build the pool and targets fixture for select_training() tests
#'
#' @description
#' The pool has `n_pool` rows split evenly into two families on a 4000 to 600
#' cm-1 grid at 4 cm-1, with `clay` measured on every row and `oc` on half of
#' them. The targets are `n_targets` pool rows (half from each family),
#' resampled onto an 8 cm-1 grid with `prospectr::resample()` and then
#' perturbed with noise, except one, `twin_id`, which is left exact so its
#' reconciled pool row is identical.
#'
#' Set `n_replicates` above zero to put replicate scans of the twinned pool
#' row into the pool as well. That is the case the gap-based twin rule could
#' not see: with two or three near-copies every distance is tiny, so the gap
#' between first and second nearest never opens.
#'
#' @param n_pool [Integer.] Pool rows before replicates. Default: `300`.
#' @param n_targets [Integer.] Target rows, even. Default: `8`.
#' @param seed [Integer.] Default: `1`.
#' @param target_range [Numeric, length 2.] `c(max, min)` of the target grid.
#'   Default: `c(4000, 600)`, inside the pool. Use a wider range to trigger
#'   the coverage stop.
#' @param n_replicates [Integer.] Extra pool rows that are the twinned pool
#'   row plus `replicate_noise`, appended after the `n_pool` ordinary rows.
#'   Default: `0`, which leaves the fixture and its RNG stream untouched.
#' @param replicate_noise [Numeric.] SD of the noise on a replicate scan.
#'   Default: `5e-4`, the order of a real repeat measurement.
#' @param target_scale [Numeric.] Multiplier on the target spectra, applied
#'   after resampling. Use `100` to fake a percent-against-fraction unit
#'   mismatch. Default: `1`.
#'
#' @return [List.] `pool` and `targets` (`horizons_data`), `twin_id`
#'   (the target that is an exact copy of a pool row), `twin_pool_id`,
#'   `replicate_pool_ids` (the replicate scans of `twin_pool_id`, empty when
#'   `n_replicates = 0`), `family_of_target` (named integer, 1 or 2),
#'   `pool_wn`, `target_wn`.
#' @noRd
make_select_fixture <- function(n_pool          = 300,
                                n_targets       = 8,
                                seed            = 1,
                                target_range    = c(4000, 600),
                                n_replicates    = 0,
                                replicate_noise = 5e-4,
                                target_scale    = 1) {

  set.seed(seed)

  n_replicates <- as.integer(n_replicates)

  ## Pool spectra ----------------------------------------------------------

  pool_wn <- seq(4000, 600, by = -4)
  n_half  <- n_pool %/% 2

  ## The twinned row has to be known before the pool matrix is assembled when
  ## replicates are wanted. Drawing it here would shift the RNG stream for
  ## every existing fixture, so the draw only moves when it has to.
  twin_pick <- if (n_replicates > 0L) sample(seq_len(n_half), 1) else NULL

  fam1 <- gaussian_family(n_half,          pool_wn, centres = c(3400, 2920, 1630, 1030))
  fam2 <- gaussian_family(n_pool - n_half, pool_wn, centres = c(3620, 2515, 1420, 870))

  pool_mat <- rbind(fam1, fam2)

  pool_ids <- sprintf("P%03d", seq_len(n_pool))
  family   <- rep(c(1L, 2L), c(n_half, n_pool - n_half))

  replicate_pool_ids <- character()

  if (n_replicates > 0L) {

    reps <- matrix(pool_mat[twin_pick, ], nrow = n_replicates, ncol = length(pool_wn), byrow = TRUE) +
            matrix(stats::rnorm(n_replicates * length(pool_wn), sd = replicate_noise), nrow = n_replicates)

    replicate_pool_ids <- sprintf("R%03d", seq_len(n_replicates))

    pool_mat <- rbind(pool_mat, reps)
    pool_ids <- c(pool_ids, replicate_pool_ids)
    family   <- c(family, rep(family[twin_pick], n_replicates))
    n_pool   <- n_pool + n_replicates

  }

  colnames(pool_mat) <- paste0("wn_", pool_wn)

  pool_tbl <- dplyr::bind_cols(
    tibble::tibble(sample_id = pool_ids, family = family),
    tibble::as_tibble(pool_mat)
  )

  ## spectra() and add_response() print their progress tree unconditionally;
  ## keep the test log quiet.
  utils::capture.output(pool <- spectra(pool_tbl))

  ## Responses: clay complete, oc on half the rows -------------------------

  lab <- tibble::tibble(
    sample_id = pool_ids,
    clay      = round(stats::runif(n_pool, 5, 60), 1),
    oc        = round(stats::runif(n_pool, 0.2, 6), 2)
  )
  lab$oc[sample(n_pool, n_pool %/% 2)] <- NA_real_

  utils::capture.output(pool <- add_response(pool, lab, variable = c("clay", "oc")))

  ## Targets: fresh draws from the two families, plus one exact pool copy --
  ## Fresh draws (not pool rows plus noise) so that only the twin is a
  ## replicate; a pool row with small noise is exactly what the twin check
  ## is meant to catch.

  n_fresh    <- n_targets - 1L
  n_fresh1   <- n_fresh %/% 2
  n_fresh2   <- n_fresh - n_fresh1

  if (is.null(twin_pick)) twin_pick <- sample(seq_len(n_half), 1)

  src <- rbind(
    pool_mat[twin_pick, , drop = FALSE],
    gaussian_family(n_fresh1, pool_wn, centres = c(3400, 2920, 1630, 1030)),
    gaussian_family(n_fresh2, pool_wn, centres = c(3620, 2515, 1420, 870))
  )
  target_family <- c(1L, rep(1L, n_fresh1), rep(2L, n_fresh2))

  target_wn <- seq(target_range[1], target_range[2], by = -8)

  ## prospectr wants increasing axes; flip back after
  tgt <- prospectr::resample(X = src[, order(pool_wn), drop = FALSE], wav = sort(pool_wn),
                             new.wav = sort(target_wn), interpol = "spline")
  tgt <- tgt[, rev(seq_len(ncol(tgt))), drop = FALSE]
  tgt <- tgt * target_scale
  colnames(tgt) <- paste0("wn_", target_wn)

  twin_row   <- 1L
  target_ids <- sprintf("T%02d", seq_len(n_targets))

  targets_tbl <- dplyr::bind_cols(
    tibble::tibble(sample_id = target_ids),
    tibble::as_tibble(tgt)
  )

  utils::capture.output(targets <- spectra(targets_tbl))

  family_of_target <- stats::setNames(target_family, target_ids)

  list(
    pool               = pool,
    targets            = targets,
    twin_id            = target_ids[twin_row],
    twin_pool_id       = pool_ids[twin_pick],
    replicate_pool_ids = replicate_pool_ids,
    family_of_target   = family_of_target,
    pool_wn            = pool_wn,
    target_wn          = target_wn
  )

}
