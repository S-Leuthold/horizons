...
- ✅ **M3.1 Residual-Based UQ**: `R/library-uq.R` (38KB, 8 functions) implements `train_cluster_models_with_uq()`, `predict_with_uq()`, `predict_quantiles()`, `repair_quantile_crossings()` - fixed critical OOF bias bug (26× impact on interval width)
- ✅ **M3.2 Heteroscedasticity Validation**: Spike tests confirmed quantile models needed (p<0.05 across pH, OC, total_carbon)
- 🚧 **M3.3 Conformal Calibration** (INCOMPLETE): Infrastructure in place (`compute_conformal_margin()`, c_alpha integration) but needs OOF quantile extraction via shared CV folds - detailed spec in `M3.3_CONFORMAL_IMPLEMENTATION_SPEC.md`
- 📋 **Recent Commits**: f38581b (Session 6 cleanup), 7ce50e9 (residual-based implementation), 2c7fc4c (texture g/kg fix)
...
2. **Phase Sequencing**: Continue Phase 3 UQ (M3.3→M3.4) or pivot to Phase 2 polish (water bands, RPIQ composite scoring)?
