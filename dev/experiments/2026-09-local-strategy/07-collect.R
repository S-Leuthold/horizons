...
for (property in EXP_PROPERTIES$property) {
  for (pr in pairs) {
    ...
    ## NOTE: these must be plain locals with names distinct from the tibble
    ## columns below. tibble() evaluates sequentially and lets a later column
    ## refer to an earlier one, so naming the bootstrap vector `d_rpd` made
    ## `quantile(d_rpd, ...)` read the just-defined point estimate instead —
    ## which is why the 2026-09-11 run reported lo == hi == point, se = NA.
    bs_rpd <- bs_rmse <- numeric(DRAWS)
    for (b in seq_len(DRAWS)) {
      i <- sample.int(n, n, replace = TRUE)
      bs_rpd[b]  <- rpd_vec(j$truth_s[i], j$.pred_s[i]) - rpd_vec(j$truth_s[i], j$.pred_r[i])
      bs_rmse[b] <- yardstick::rmse_vec(j$truth_s[i], j$.pred_s[i]) - yardstick::rmse_vec(j$truth_s[i], j$.pred_r[i])
    }
    boot_rows[[length(boot_rows) + 1]] <- tibble(
      ...
      d_rpd_lo  = unname(quantile(bs_rpd, 0.025)),
      d_rpd_hi  = unname(quantile(bs_rpd, 0.975)),
      d_rpd_se  = sd(bs_rpd),
      d_rmse    = yardstick::rmse_vec(j$truth_s, j$.pred_s) - yardstick::rmse_vec(j$truth_s, j$.pred_r),
      d_rmse_lo = unname(quantile(bs_rmse, 0.025)),
      d_rmse_hi = unname(quantile(bs_rmse, 0.975))
    )
  }
}
...
## NOTE: get() takes ONE strategy; building this line with the whole vector
## of names is what crashed collect on the 2026-09-11 run (switch() needs a
## length-1 EXPR). Build it per strategy.
status <- sapply(c("A", "B", "C", "D", "E"), function(s) if (!have(s)) "missing" else if (eligible(s)) "eligible" else "INELIGIBLE (coverage)")
elig_lines <- vapply(names(status), function(s) {
  if (status[[s]] == "missing") return(paste0("- ", s, ": missing"))
  cv <- get(s, "coverage_90")
  paste0("- ", s, ": ", status[[s]], " — coverage ",
         paste(ifelse(is.finite(cv), sprintf("%.3f", cv), "—"), collapse = " / "))
}, character(1))
lines <- c(lines, "## Eligibility", "",
           paste0("Properties in order: ", paste(props, collapse = " / "), "."), "",
           unname(elig_lines), "")
