...
* **`validate_horizons_data()` now runs at the end of every data verb** (#24): `spectra()`, `parse_ids()`, `average()`, `add_response()`, `configure()` and `validate()`, not just `standardize()` and `select_training()`. A `spectra()` tibble with duplicate `sample_id` values, or predictor columns out of wavenumber order, used to build a `horizons_data` object the validator would only catch once it reached `standardize()` or `select_training()`; it is now caught where it is built. `spectra()` also now sorts predictor columns to decreasing wavenumber order itself, the same way `standardize()`'s no-op path already did, so a library stored increasing (the KSSL snapshot, for one) is valid immediately rather than only after `standardize()`. `evaluate()`, `fit()` and `ensemble()` already validated their own slot on return and are unchanged.
