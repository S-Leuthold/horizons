...
* **`average()`'s QC report no longer counts a group whose every replicate failed QC as clean** (#89). Under the default `on_all_outliers = "warn"` such a group is averaged with all its replicates, so it loses none, and the report counted clean groups as the groups that lost none: it printed "all 2 groups clean" directly under the warning that a sample's replicates had all failed. The report now gives each kind of group one line, so the lines add up to the number of groups: the clean groups, the groups that lost some replicates and averaged the rest, and the groups whose every replicate failed, with what `on_all_outliers` did to them ("averaged anyway", "one replicate kept, 2 removed", or "dropped, 3 replicates removed"). The line counting groups with removed replicates now counts the partly failed groups only; what a wholly failed group lost under `"keep_best"` or `"drop"` is on its own line, which replaces "groups dropped entirely".
