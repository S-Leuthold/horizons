...
* **`step_select_boruta()` has no fallback selector: when Boruta fails, the step fails** (#75). `prep()` aborts with class `horizons_boruta_error`, carrying Boruta's own message, where it used to keep the 50 columns most correlated with the outcome under the `"boruta"` label. `evaluate()` and `fit()` record such a configuration as failed. That includes Boruta not being installed (it is suggested, not imported), which also used to fall back silently. When Boruta runs but confirms nothing, the step now warns with class `horizons_boruta_warning`; the selection is what it was: every variance-filtered column when Boruta rejected them all, as the other selection steps keep all their columns when they select nothing, and otherwise the attributes it left tentative. Selections change for every configuration whose Boruta run used to abort into the fallback, which on small or noisy data was common (see Bug Fixes).
