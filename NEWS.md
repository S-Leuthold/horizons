...
* **`configure()` gains `outcome_range`, so signed outcomes can be fit** (#76). Every prediction the package scored or served was floored at zero, and `fit()`'s response bound was the largest training outcome times 1.5, validated as positive. A property that is never positive (δ13C, around -30 to -15 ‰) therefore ran all of `fit()`'s tuning and then failed validation, and one whose maximum happened to be positive was fit with its negative predictions clamped to zero and scored on the clamped values. `outcome_range = c(lower, upper)` is the outcome's physical range, either end possibly infinite, set once per object and recorded in `config$outcome_range`. Every clamp reads it: the test-set, out-of-fold and UQ-calibration predictions `evaluate()` and `fit()` score, the tuning metrics of a transformed response, `predict()`'s point predictions and interval bounds, and the ensemble's combined, out-of-fold, fold-model and interval predictions. The lower bound replaces the zero floor, with no floor at `-Inf`, and a finite upper bound caps. The response bound is now `max + (RESPONSE_BOUND_MARGIN - 1) * (max - anchor)`, capped at a finite upper bound, where the anchor is the range's lower bound, or the training minimum when that bound is `-Inf`. It lies above the training maximum whatever the sign, and under the default range it is identical to `max * 1.5` for every normal double, so existing fits and pinned tests are unchanged. `validate_horizons_fit()` accepts any finite bound above the range's lower bound and no higher than its upper bound. The default, `c(0, Inf)`, is the behaviour before the argument existed, and objects configured or fitted by an earlier version read as it. The range is the property's physical range, set from domain knowledge rather than from the data, and `-Inf` is better than an arbitrarily low finite floor, which loosens the response bound. `configure()` refuses an outcome with an observed value outside the range, or an infinite one, with class `horizons_input_error` and a message naming `outcome_range` and how to set it, and `evaluate()`, `fit()` (from `evaluate()` or cold) and `ensemble()` repeat the check before they draw a split or fit anything. A range with a negative lower bound cannot be combined with the `log`, `log10` or `sqrt` transformations, which are undefined below 0 or -1 and whose back-transforms floor the predictions (`configure()` refuses the pair), and `evaluate()` and `fit()` refuse to rank by `rrmse` under it, since the mean outcome it divides by can be zero or negative. One range applies to the one outcome an object models. The range is one of the tuning settings `evaluate()` stamps on every result and checkpoint row (#42): a rerun into the same `output_dir` under another range refuses to resume, rows written before the range was recorded resume only under the default range (they were scored with the zero floor), and `fit()` refuses an evaluation whose rows record a different range. `back_transform_predictions()` gains `outcome_range`, and its `upper_bound` must lie above the range's lower bound rather than above zero; `evaluate_single_config()`, `fit_single_config()`, `fit_uq()` and `tuning_metric_set()` take the range as an argument defaulting to `c(0, Inf)`.
...
* **`step_select_boruta()` has no fallback selector: when Boruta fails, the step's `prep()` fails** (#75). A Boruta error no longer produces results labelled Boruta from a correlation filter; the step used to keep the 50 columns most correlated with the outcome in its place. The abort carries Boruta's own message and class `horizons_boruta_error` in its condition chain. Through `recipes::prep()` the outer condition is recipes' `recipes_error_step`, so test for the class with `rlang::cnd_inherits()`, not `inherits()`. Inside `evaluate()` and `fit()` a configuration is recorded as failed when Boruta fails on the full training set or on every fold. When it fails on some folds only, tune keeps those errors in its notes and scores the folds that ran; making fold-level failures visible is #96. Boruta is suggested, not imported, and a missing install used to fall back silently as well; `prep()` now checks for it first with `rlang::check_installed()`, which names the package. When Boruta runs but confirms nothing, the step now warns with class `horizons_boruta_warning`; the selection is what it was: every variance-filtered column when Boruta rejected them all, as the other selection steps keep all their columns when they select nothing, and otherwise the attributes it left tentative. Selections change for every configuration whose Boruta run used to abort into the fallback, which on small or noisy data was common (see Bug Fixes).

* **`validate()` no longer removes response outliers; `evaluate()` trims them from its training partition** (#77). With `remove_outliers = "response"` (or `TRUE`), `validate()` computed Tukey fences over every row and removed the rows outside them before any split existed, so `evaluate()`'s test set lost exactly its hardest cases, chosen by their own labels, and the reported test RMSE and RPD described a population trimmed of its extremes. **Behaviour change:** `validate()` now removes only spectral outliers (the detector reads no labels). Response outliers are still flagged over the whole table, for information (`response_ids`, check P006), and a request for response removal is recorded in `validation$outliers$response_trim` (the outcome, method and threshold). `evaluate()` draws its train/test split on the untrimmed rows, computes the fences from the training partition's labels alone, and drops the training rows outside them before the CV folds are drawn; test rows are never removed on their labels, so the test metrics include the extremes the model will meet. `evaluation$response_trim` records the fences, the training rows they were computed over and the trimmed ids, `n_train` counts the rows left, and both console trees say how many training rows were trimmed. `fit()` reuses that record rather than recomputing fences, so it fits without exactly the rows `evaluate()` left out, and its cold start from one configuration trims through the same helper. With UQ or AD on, `fit()` draws its calibration set from the untrimmed training part (the trimmed rows are training rows, never test rows) and drops the trimmed rows from the fit rows only: split conformal needs calibration rows exchangeable with the rows it will be asked about, extremes included, and a trimmed pool undercovered them without saying so, since the out-of-fold coverage is read on trimmed rows too. The degradation check compares the CV RPD with the test RPD over the test rows inside the trim's fences, since the CV ran inside them, and `degraded_reason` says so; the reported test metrics stay untrimmed. `models$response_bound` counts the trimmed training rows, so the deploy-time clamp never sits below a value observed in the training partition. An ensemble's CV+ intervals are built from the members' out-of-fold predictions, which cover only the rows inside the fences, so `predict(interval = TRUE)` on an ensemble whose members were trimmed warns, with class `horizons_response_trim_warning`, that its intervals were calibrated within them. `summary()` gives the number of trimmed training rows on the Split line of a fit. Under `remove_outliers = TRUE` a row that is both kinds of outlier is removed as `"spectral"`; `removal_detail` rows this version writes always have that reason. The CV assessment folds come from the trimmed training rows, so the cross-validated metrics (and the ranking on them) describe the trimmed population; the test metrics do not. Re-configuring clears the request with the rest of the verdict, since it was made for the outcome being replaced, so `validate()` must run again after `configure()` to trim the new outcome; before, the rows removed for the old outcome stayed removed. The trim is part of the tuning settings `evaluate()` records on every checkpoint (`response_threshold`, `NA` when none was requested), so a rerun into the same `output_dir` with a different trim refuses to resume, naming it, and rows written before it was recorded resume as unverified. An object validated by an earlier version, whose rows are already gone, still evaluates and fits: its removal record names them (`reason` `"response"`; a `"both"` row would have gone as a spectral outlier anyway, and is not counted), and `evaluate()` and `fit()` warn, with class `horizons_response_trim_warning`, that its test metrics exclude them. To score such an object on untrimmed test rows, start from the object before that `validate()` call.
...
* **`validate_horizons_data()` runs at the end of every data verb, but a duplicate or `NA` `sample_id` only warns before `average()` and aborts from `average()` on** (#24). `spectra()`, `parse_ids()` and `standardize()` run before replicate scans have been collapsed to one row per sample, so more than one row legitimately sharing a `sample_id` is normal there — Bruker's own OPUS naming does this, `S100-1.0` and `S100-1.1` both becoming `S100-1` once the extension is stripped — and `spectra() |> standardize() |> parse_ids(...) |> average(by = "sample_id")` is the documented chain. An earlier version of this change made `spectra()` and `parse_ids()` abort on exactly the pre-`average()` state they are supposed to tolerate, which broke that chain on real OPUS data (15 scans, 4 samples, aborting at `parse_ids()`) and on `average()`'s own contract, since a duplicate `sample_id` is what tells it there are replicates to collapse. The validator now takes a `stage` argument: `spectra()`, `parse_ids()` and `standardize()` call it with `stage = "raw"`, which downgrades a duplicate or `NA` `sample_id` to a warning (the wording matches the validation spec's A004 check) and skips the predictor-NA check entirely — a value outside `standardize()`'s eventual trim range is legitimately still `NA` before the trim runs, and `standardize()`'s own non-finite check still catches anything that survives an actual trim or resample. `average()` onward (`average()`, `add_response()`, `configure()`, `validate()`, `select_training()`, and every other caller) keeps the default `stage = "full"`, where the same conditions abort, naming the ids and pointing at `average()`. **Breaking:** an object saved by an earlier version whose predictor columns are stored increasing now fails a `configure()` re-run (or any other full-stage verb) with a wavelength-order error where it previously passed silently; re-run `standardize()` or `spectra()` on it first to sort the axis. `spectra()` also now sorts predictor columns to decreasing wavenumber order itself, the same way `standardize()`'s no-op path already did, printing a "Sorting" console line when it does, so a library stored increasing (the KSSL snapshot, for one) is valid immediately rather than only after `standardize()`. The wavelength-order check itself now names the problem where it can rather than a generic "must be strictly decreasing": two predictor names parsing to the same wavenumber (`600` and `600.0`) are reported as duplicates, and a name that matches the wavenumber pattern but does not parse as one (two decimal points) is reported as unparseable, instead of reaching `diff()`/`all()` as an `NA` and crashing with "missing value where TRUE/FALSE needed". `evaluate()` and `fit()` now also certify the base data contract, at entry (see the next item); `ensemble()` still only validates its own slot on return and is unchanged.

* **`evaluate()` and `fit()` now certify the base data contract at entry, and `standardize()` and `average()` certify it before rebuilding `data$analysis`, closing the last unregistered-column gap in the pipeline** (#24). `validate_horizons_eval()` and `validate_horizons_fit()` check the evaluation and models slots on return; neither ever checked the base contract, so a column landing in `data$analysis` after `configure()` — a later `parse_ids()`, or a direct assignment — reached `build_recipe()`'s `outcome ~ .` as an unregistered predictor, undetected, all the way through model fitting. `evaluate()` and `fit()` now call `validate_horizons_data()` (`stage = "full"`) as their first step. Separately, `standardize()`'s and `average()`'s own end-of-verb call caught the same gap on *return*, but each rebuilds `data$analysis` from `role_map`'s predictor and non-predictor columns before it gets there (standardize()'s Steps 7-9, average()'s Step 8), so a column absent from `role_map` was silently dropped from the rebuilt table rather than surfaced; both now also validate (`stage = "raw"`) at entry, before the rebuild, so the column is refused instead. Two smaller fixes to the same validator: a non-finite (`Inf`) predictor value is now skipped at `stage = "raw"` the same way `NA` already was, for the same reason — it is legitimately still there before `standardize()`'s trim runs, and `standardize()` itself aborts on anything non-finite that survives trimming and resampling; and the duplicate-`sample_id` message is truncated to the first 5 ids plus a count of the rest, matching every other list in this validator, rather than printing an unbounded list at library scale.

* **`validate_horizons_data()` now requires exactly one `role_map` row per `analysis` column**, in both stages. Presence was already checked — every `analysis` column has to appear in `role_map` at least once — but a column named twice, with different roles or the same one, was not: it made `role_map$role[role_map$variable == x]` ambiguous to every consumer that expects a scalar, and let the column fall through `build_recipe()`'s `outcome ~ .` as an unregistered predictor rather than failing loudly.
...
* **Failed and warned configs record their cause** (#96). A recipe step error reached the results of `evaluate()` and `fit()` with nothing after the stage name. recipes wraps every step error in a condition whose `$message` is empty, with the text in its parent chain, and the per-stage capture read `$message`, so a Boruta failure was stored as `"Parameter finalization failed: "`. Every capture site now goes through one internal helper, `condition_summary()`, which records the root cause of the chain on one line, followed by the step it happened in, as in ``"Parameter finalization failed: boom from step (in `step_mutate()`)"``. The same text reaches the all-failed aborts of `evaluate()` and `fit()` (#85), the interval and AD warnings from `predict()`, the unreadable-checkpoint warning, `handle_results()`'s aborts, and the `spectra()` and `standardize()` messages that quoted an upstream error. When every model fails inside tuning, tune returns rather than erroring and keeps the reason in `.notes`, so `evaluate()`'s grid search stopped at "all models failed during CV" and `fit()`'s warm-start re-tune pointed at `show_notes()`. Both now read the most frequent error note back and append it, with how many folds raised it. tune also keeps the warnings raised inside its resampling in `.notes`, where the capture never saw them: a recipe step that warned during prep left the config with `status = "success"` and no warnings, unless a model with `mtry` happened to prep the recipe outside tune while finalizing it. The warning notes from `tune_grid()`, `tune_bayes()`, `last_fit()`, and `fit()`'s warm-start tuning and out-of-fold resampling now join the config's `warnings`. The `warnings` of `evaluate()`'s results and `fit()`'s members are now one cleaned line per distinct message, where they were every captured warning as cli rendered it, repeated once per signal: a warning captured in full and the notes tune kept for it are one entry, as are the variants of a message that differ only in a number. Each entry says how many folds raised it, `test set` when the final fit on the test set did (so a test-set signal is never read as a cross-validation one), and `numbers vary` for merged variants. The package's own notices come first, then the five most frequent messages and a line counting the rest. When `last_fit()`'s final fit fails, `evaluate()` still records the configuration as it did, with missing test metrics, but its `warnings` now say why (`"Test-set fit failed: ..."`). A cause longer than 300 characters is cut at the end and marked with `...`; the location stays. A `fit()` member that fails because every model in its re-tune failed now records how the re-tune started (`warm_start`, `start_grid_size`) like any member that reached tuning; both were `NA`.

* **`average()`'s QC report no longer counts a group whose every replicate failed QC as clean** (#89). Under the default `on_all_outliers = "warn"` such a group is averaged with all its replicates, so it loses none, and the report counted clean groups as the groups that lost none: it printed "all 2 groups clean" directly under the warning that a sample's replicates had all failed. The report now gives each kind of group one line, so the lines add up to the number of groups: the clean groups, the groups that lost some replicates and averaged the rest, and the groups whose every replicate failed, with what `on_all_outliers` did to them ("averaged anyway", "2 replicates kept, 4 removed", or "dropped, 6 replicates removed", counted over every such group). The line counting groups with removed replicates now counts the partly failed groups only; what a wholly failed group lost under `"keep_best"` or `"drop"` is on its own line, which replaces "groups dropped entirely". A group of one scan, which QC has nothing to correlate against, was counted clean too; it is now counted on its own line as a single-replicate group, not QC'd.

* **The console says "stratified" only when a draw was stratified** (#91). `evaluate()`'s split line printed "(80/20, stratified)", and `fit()`'s "CV: N-fold stratified on <outcome>", whatever the draw did. A draw asked to stratify can come out unstratified two ways. The stratified call can fail and be retried without strata, which a note said. More often rsample drops the strata itself, without an error: `make_strata()` bins a numeric outcome into quartiles only while each bin holds 20 rows, so below 40 rows it warns "Too little data to stratify" and draws unstratified, and it draws unstratified with no warning at all when a few-valued outcome pools into one stratum or tied quantiles leave one bin, as an outcome more than three-quarters zero does. The returned object names the outcome as its strata either way. Every draw now asks `make_strata()` how many strata it forms, passing it and the draw the same `breaks` and `pool` (rsample's defaults, now explicit), and the tree says "stratified" or "unstratified" on `evaluate()`'s split and Tuning lines, `fit()`'s CV line, the split line of a cold start, and `fit()`'s calibration line. The folds are drawn on the training part, so at 45 rows the split is stratified and the folds are not. The calibration line said neither before, and printed only with UQ on; it is now "UQ and AD calibration", or whichever of the two is on, and a failed stratified calibration split, which used to fall back silently, gets a note under it. The draws themselves are unchanged.

* **Console notes print inside the tree, under the line they qualify** (#91). `fit()` printed its "Requested n_best", "Stratified split failed", "Calibration set too small" and "Stratified CV failed" notes before its `┌ fit` header; `evaluate()` its two "Stratified ... failed" notes and, on a resume, "Loaded N checkpointed results" and the notes on dropped stale or earlier-schema checkpoint rows before `┌ Evaluation`; and `add_response()` its low-match hints before its "Adding response data" line, so each sat outside the tree it belonged to. The checkpoint notes now sit under the Configs line, whose "(N from checkpoint)" already gave the loaded count, so the Loaded line is gone. `add_response()`'s hints now nest under its Matched line, which is yellow when fewer than half the samples matched, and no longer repeat the matched count.

* **`step_select_boruta()` runs Boruta to completion instead of silently becoming a correlation filter** (#75). The step set ranger's `mtry` once, from all the cluster representatives, and passed it to every Boruta run. Boruta hands each run only the attributes it has not yet rejected plus as many shadow copies, so once fewer than `mtry / 2` attributes remained ranger aborted, and the step caught the abort and kept the 50 columns most correlated with the outcome, saying so only in a console message. On small or noisy data that was common: the review that found it saw it in every Boruta case on the test fixtures. Each run now clamps `mtry` to the columns it is given. The `ntree` and `mtry` the step passed to `Boruta()` never reached its custom importance source, so they did nothing; they are gone, and the forest's settings (250 trees, `mtry` of `round(2 * sqrt(n))` for `n` representatives, at least 5 and at most `n - 1`) are set where ranger is called. The trained step records what Boruta did in its `boruta` element: the confirmed, tentative and rejected counts, each attribute's decision, the runs, and the `mtry` of each run. `print()` shows the counts.
...
* **`select_training()` records the axis it returns** (#90). The return is built from the pool resampled onto the targets' grid, but it carried the pool's `provenance$standardization`, so a pool standardized at 4 cm-1 and drawn around targets at 8 recorded a grid step of 4 on columns 8 apart, and anything reading the grid from the record got the wrong spacing. When the pool is resampled, the record now describes the returned columns: `grid` is the targets' (`NULL` when they are not on a canonical grid, or when their recorded grid does not match their columns), `n_wavelengths` and `wavelength_range` are the new axis's, and `resampled` is `TRUE`. The `standardize()` arguments (`resample`, `trim`, `remove_water`, `baseline`) and `applied_at` stay the pool's, because they produced the values, so `resample` can differ from `grid$step` and `trim` from `wavelength_range`, as `resample` already could after a `force = TRUE` no-op call. With `baseline = TRUE`, the hull was fitted on the pool's grid and the corrected spectra then interpolated. A new `reconciliation` entry records the move: `operation`; `clamp`, the overshoot at each end when an end column was taken at the pool's endpoint rather than at the grid point it is named for (else `NULL`); and `pool`, the pool's record as it was, whose `grid` is the pool's original grid. It is the object's own axis history, distinct from `selection$reconciliation`, which records how the two axes compared, and a `force = TRUE` no-op `standardize()` call carries it forward with the grid. A pool already on the targets' grid keeps its record unchanged, and a pool that was never standardized still has none. `configure()` and `evaluate()` read the Savitzky-Golay window's width from the axis itself (#62) and are unaffected; on a new selection, the recorded step they would fall back to now agrees with that axis. An object selected before this change keeps the pool's record; re-run `select_training()` to rewrite it.
